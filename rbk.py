...
from slack_sdk import errors as slack_sdk_errors
import requests
import configparser
...
def send_to_slack(message, channel):
    try:
        # Thêm một giãn cách thời gian giữa các lần gửi
        t.sleep(1)
        slack_client.chat_postMessage(channel=channel, text=message)
    except slack_sdk.errors.SlackApiError as e:
        # Xử lý lỗi và thông báo ra màn hình
        ...
        if response.status_code == 200:
            ...
            for element in a_cau_elements:
                ...
                if value not in unique_numbers:  # Check for duplicates
                    ...
                    cau_keo_str = ','.join(element.strip("'") for element in unique_numbers)
